...
then
  ...
  then
    ...
    #shellchec -x --enable=all --format=diff --shell=sh $SH_SCRIPTS >> shellcheck-fixes.diff
    #shellchec -x --enable=all --format=diff --shell=bash $BASH_SCRIPTS >> shellcheck-fixes.diff
